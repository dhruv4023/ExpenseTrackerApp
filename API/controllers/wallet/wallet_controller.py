...
from database.wallet import createWallet
...
async def create_wallet(req: Request, token: str = Depends(verify_token)):
    try:
        if(createWallet(userName=token["username"], methodName=get_user_date(token["username"]).json()["data"]["user"]["firstName"]+"'s Transactions")):
